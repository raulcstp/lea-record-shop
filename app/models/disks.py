...
class Disks(db.Model):
    ...
    release_year = db.Column(db.String(4), nullable=False, index=True)
...
class DisksSchema(Schema):
    ...
    visibility_date = fields.DateTime(
        error_messages={"invalid": "Not a valid date with format yyyy-mm-ddThh:mm:ss"}
    )
...
disk_filter_schema = DisksSchema(
    exclude=("id", "created_at", "updated_at", "visibility_date"), partial=True
)
